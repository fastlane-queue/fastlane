...
def test_monitor_job_with_retry(client):
    ...
    with client.application.app_context():
        ...
        queue = Queue(
            'monitor', is_async=False, connection=client.application.redis)
        result = queue.enqueue(job_mod.monitor_job, t.task_id, job_id,
                               ex.execution_id)
        ...
        nj = app.redis.zrange('rq:scheduler:scheduled_jobs', 0,
                              0)[0].decode('utf-8')
        ...
        t.reload()
        expect(t.jobs[0].executions[0].status).to_equal(Job.Status.done)
