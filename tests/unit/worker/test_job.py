...
def test_downloading_image1(worker):
    ...
    with app.app_context():
        ...
        expect(result).to_be_true()
        expect(execution.status).to_equal(JobExecution.Status.pulling)
...
def test_run_container2(worker):
    ...
    with app.app_context():
        ...
        exec_mock = MagicMock()
        exec_mock.run.side_effect = HostUnavailableError("docker", "9999", "failed")
...
def test_run_container3(worker):
    ...
    with app.app_context():
        ...
        exec_mock = MagicMock()
        exec_mock.run.side_effect = RuntimeError("test")
